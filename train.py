...
def get_final_result(Results,inforamtion,fold):
    ...
    for threshold in acc:
        print(f"Fold {fold}: Combined accuracy within {threshold}ms: {acc[threshold]}")

def train_test(args,train_data_loader, val_data_loader,train_loss,val_loss,size, fold, model, optimizer,scheduler):
    ...
    plt.legend(loc='best')
    plt.title(f'Patch-DAT  loop-fold-{fold}-mean absolute error')
    plt.show()
    return df_


def main():
    cv = KFold(n_splits=9, shuffle=True, random_state=args.Random_num)
    fold = 0
    # 9-fold cross-validation
    for train_index, val_index in cv.split(X_train_ori, y_train_ori):
        fold += 1
        print("The fold {} of CV".format(fold))
        print("-----------------------------------------------------------------------")
        X_train, X_val = X_train_ori[train_index], X_train_ori[val_index]
        y_train, y_val = y_train_ori[train_index], y_train_ori[val_index]

        y_train, y_val = label_extract(y_train), label_extract(y_val)
        y_train, y_val = data_reshape(y_train), data_reshape(y_val)

        info_all = data_reshape(info)
        info_all = np.array(info_all).astype(int)
        info_all = torch.from_numpy(info_all)

        info_train = info_train_ori[train_index]
        info_val = info_train_ori[val_index]
        info_val = data_reshape(info_val)
        info_train = data_reshape(info_train)
        X_train, X_val = reshape_and_scaling3(X_train, X_val, Window_flag=args.Window_flag, Normalization=args.Normalization,type=args.normalization_type)
        ...
        info_val = np.array(info_val).astype(int)
        info_val = torch.from_numpy(info_val)
        info_train = np.array(info_train).astype(int)
        ...
        val_dataset = Data.TensorDataset(X_val, y_val, info_val)
        ...
        Results = train_test(args,train_data_loader, val_data_loader,train_loss,val_loss,size, fold=fold, model=model, optimizer=optimizer,scheduler=scheduler)  # Obtain training and testing results
        get_final_result(Results,info_all,fold)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--learning_rate', type=float,default=0.0038)
